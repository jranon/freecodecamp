...
5.  Got some feedback and fixed the following: calculator allowed multiple periods in decimals; equals sign, when pressed twice, would throw an error in the console; dividing by zero now throws an error instead of continuing the chain of functions and returning an incorrect answer; you can now clear back and edit a number and it will display in the full equation properly.  Code changed to check for veracity of variables instead of simply checking that they're undefined, to ease confusion when looking at the code and to reduce chances of errors if changes are made.